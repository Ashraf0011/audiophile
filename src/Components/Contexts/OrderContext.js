...
export const OrderContextWrapper = ({ children }) => {
    ...
    const reducer = (state, action) => {
        switch (action.type) {
            // case "SEND_ORDER": {
            //     let { userName, userEmail, userPhone, userAddress, userPostCode, userCity, userCountry, paymentMethod, e_Number, e_Pin } = action.payload;
    ...
    // const sendOrder = () => {
    //     console.log("SEND_ORDER disp")
    //     dispatch({ type: "SEND_ORDER", payload: orderState })
    // }
    ...
    const setAddress = (e) => dispatch({ type: "ADDRESS", payload: e.target.value })
    const setPostCode = (e) => dispatch({ type: "POST", payload: e.target.value })
    const setCity = (e) => dispatch({ type: "CITY", payload: e.target.value })
    const setCountry = (e) => dispatch({ type: "COUNTRY", payload: e.target.value })
    const setPayment = (x) => dispatch({ type: "PAYMENT", payload: x })
    const setENUMB = (e) => dispatch({ type: "E_NUMBER", payload: e.target.value })
    ...
    return (
        <OrderContext.Provider value={{ orderState, options, handlePaymentOptions, resetForm, setName, setEmail, setPhone, setAddress, setPostCode, setCity, setCountry, setPayment, setENUMB, setEPIN }}>
            {children}
