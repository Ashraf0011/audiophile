...
const Buttons = (props) => {
    switch (props.button_type) {
        ...
        case "filled_btn":
            return (
                // 
                <Link to={props.where} style={{ width: props.width ? `${props.width}` : "160px" }} >
                    <button className='aLink filled_btn' onClick={props.onClick} style={{ width: props.width ? `${props.width}` : "160px" }} >
                        {props.name}
                    </button>
                </Link>
            )
